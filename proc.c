...
ucv_sleep(int variable, int index)
{
    // Get the current process
      ...
      if(index < 0 || index >= NLOCK) {
        panic("ucv_sleep: invalid lock index");
      }

      // Acquire the lock
      acquire(&ptable.lock);

      // Release the userspace spinlock at index
      uspinlock_release(index);

      // Set the process's chan to the variable
      ...
      // Acquire the userspace spinlock at index
